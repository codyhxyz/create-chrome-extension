...
| `cws-video` skill (wraps external) | Skill + dependency | **Planned (Session 6, default-on).** Launch-video generation for CWS listings via [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) (install: `npx skills add heygen-com/hyperframes`). Parallel to `cws-screens` — same declarative pattern (config → generate → validate presence), different asset type. Will add a `ship-ready-video` validator rule (ship-only). **Taste decision: video ships on by default.** CWS listings with a promo video convert markedly better, and the same asset doubles as a launch asset for ProductHunt / Twitter / LinkedIn. Factory clones get `video/config.ts` with placeholder content and a failing `ship-ready-video` rule by default; users who genuinely don't want a video delete `video/` (the rule no-ops on absent directory, same escape-hatch pattern as `screenshots/`). The HeyGen skill is a hard dep for the default flow (opposed to truly-optional external deps) — `cws-video` detects missing install and walks the user through `npx skills add heygen-com/hyperframes` rather than silently skipping. |
